...
StructuredBuffer<PerVertTextData> _NovaData;

v2f NovaVert(uint vertexID : SV_VertexID, uint instanceID : SV_InstanceID)
{
    NovaVertInit(instanceID, v2f, o);

    uint offsetInstanceID = _NovaDataIndices[InstanceIDToDataIndex(instanceID)];
    uint vertDataIndex = 4u * offsetInstanceID + vertexID;
    PerVertTextData textData = _NovaData[vertDataIndex];
    TransformAndLighting transformAndLighting = _NovaTransformsAndLighting[textData.TransformIndex];
